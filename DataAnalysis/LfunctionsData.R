...
"localL1inhom" <-
  ...
  {
    localK1inhom(X, lambda=lambda, wantL=TRUE, ..., rmax = rmax, 
                correction=correction, verbose=verbose, rvalue=rvalue,
...
"localK1inhom" <-
           ...
           sigma=NULL, varcov=NULL, update=TRUE, leaveoneout=TRUE)
  {
    ...
    a <- resolve.lambda(X, lambda, ...,
                        sigma=sigma, varcov=varcov,
                        update=update, leaveoneout=leaveoneout)
    result <- localKengine1(X, lambda=a$lambda, ..., rmax = rmax,
...
"localKengine1" <-
           ...
           correction = "Ripley", verbose = TRUE, rvalue = NULL)
  {
    ...
    rmax <- breaks$max
    breaks <- breakpts.from.r(seq(from = 0, to = rmax, length.out = 512))
    r <- breaks$r
